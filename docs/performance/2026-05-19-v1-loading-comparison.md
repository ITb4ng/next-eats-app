...
`next-eats-app`은 2026년 4월 4일 `v1.0.0`으로 최초 배포되었다. `v1.0.0`은 맛집 등록, Kakao Map 기반 지도 표시, 마커 표시, 좋아요, 마이페이지, 무한 스크롤 등 핵심 기능을 제공하는 초기 릴리즈였다.

초기 릴리즈는 서비스의 주요 사용자 흐름을 실제 배포 환경까지 연결했다는 점에서 의미가 있었다. 다만 README 보강을 준비하던 2026년 5월 18일 기준으로 초기 로딩 속도, 지도 렌더링 의존도, Supabase 무료 요금제 일시정지 가능성, 의존성/보안 패치, fallback UI, 성능 측정 자동화 측면에서 운영 품질 개선 과제가 다시 확인되었다.
...
Desktop Performance 평균은 `78.66`에서 `88`로 개선되었다. SEO는 `54`에서 `91`로 상승해 메타 정보, 페이지 구조, 검색 노출 관점의 개선이 확인되었다.
...
Mobile Performance는 약 `72`에서 `75`로 소폭 개선되었다. 다만 1회 측정값이므로 모바일 성능이 안정적으로 개선되었다고 단정하기보다는, 현재 시점에서 개선 가능성이 확인된 정도로 해석한다.
...
LCP가 약 2.1s 수준으로 유지되는 점은 현재 Production에서도 남은 핵심 과제다. 초기 화면 체감 속도는 개선되었지만, 지도 타일, Kakao Map SDK, 커스텀 폰트, 마커 이미지 같은 리소스가 LCP 후보와 렌더링 타이밍에 영향을 줄 수 있다.
...
- fallback, loading, empty, debug 상태가 운영 품질 관점에서 충분히 분리되어 있지 않았다.
- 성능 측정이 수동 Lighthouse에 의존하고 있어 반복 가능한 자동화 기준선이 부족했다.

## 현재 Production에서 확인된 개선점

현재 Production에서는 fallback store 표시 개선 이후 초기 화면 체감 속도가 개선되었다. Lighthouse 수치상으로도 Desktop Performance 평균과 SEO 개선이 확인되었다.
...
다만 이 개선은 모든 병목이 해결되었다는 의미가 아니다. 특히 LCP는 약 `2.1s` 수준으로 유지되고 있어 추가 개선 대상이다.
...
현재 Production은 `v1.0.0` 대비 Desktop Performance 평균과 SEO에서 개선이 확인되었다. Mobile Performance도 1회 측정 기준으로는 소폭 개선되었다.

하지만 LCP는 약 `2.1s` 수준으로 유지되고 있으며, Kakao Map과 외부/정적 리소스 의존도도 여전히 남아 있다. 따라서 현재 Production은 완성된 최적화 버전이라기보다 `v1.1.0`급 로딩 안정화 버전으로 보고, 이후 자동화된 측정 기반 위에서 LCP와 모바일 성능을 계속 개선해야 한다.
